...
import { RequestPasswordResetSchema, RequestPasswordResetDTO } from "./schema";
...
export default function ForgetForm() {
  ...
  const onSubmit = async (data: RequestPasswordResetDTO) => {
    ...
    try {
      ...
      if (response.success) {
        showToast("Reset link sent", "Check your email for the password reset link.", "default");
      } else {
        showToast("Request failed", response.message || "Something went wrong.", "destructive");
      }
    } catch {
      showToast("Request failed", "Something went wrong.", "destructive");
    } finally {
      setLoading(false);
    }
  };
