...
export class ParkingSpotController {
  async getParkingSpots(request: NextRequest) {
    await connectDB();
  ...
  async createParkingSpot(request: NextRequest) {
    ...
    try {
      ...
      if (error && typeof error === "object" && "issues" in error) {
        ...
        const message = issues.issues.map((issue) => `${issue.path.join(".") || "field"}: ${issue.message}`).join(", ");
  ...
  async getParkingSpot(request: NextRequest, id: string) {
    await connectDB();
  ...
  async updateParkingSpot(request: NextRequest, id: string) {
    const adminError = await requireAdmin(request);
    if (adminError) return adminError;
  ...
  async deleteParkingSpot(request: NextRequest, id: string) {
    const adminError = await requireAdmin(request);
    if (adminError) return adminError;
